...
$lineUpID = "CAN-0009356-X";      // Set this to ID of the Line Up data you want to extract
$days     = 2;                   // Number of days worth of guide data to collect (8 days max)
// whitelist of channels to scrape
// to keep requests light(er)
$include_channels = [
    '14492', // BC Knowledge Network
];
...
$json = file_get_contents($lineup_url, false, $context);

$lineup_data = json_decode( $json, true );

// filter lineup data to only include the whitelisted files
$lineup_data = array_filter($lineup_data, function($channel) use ($include_channels) {
    return in_array($channel["stationId"], $include_channels);
});
...
for ( $day = 0; $day < $days; $day++)
{
    ...
    $json = file_get_contents( $listing_url, false, $context );
